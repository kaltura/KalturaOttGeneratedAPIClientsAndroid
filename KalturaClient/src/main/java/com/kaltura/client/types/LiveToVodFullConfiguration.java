...
public class LiveToVodFullConfiguration extends ObjectBase {
    ...
    public static final Creator<LiveToVodFullConfiguration> CREATOR = new Creator<LiveToVodFullConfiguration>() {
        @Override
        public LiveToVodFullConfiguration createFromParcel(Parcel source) {
            return new LiveToVodFullConfiguration(source);
        }

        @Override
        public LiveToVodFullConfiguration[] newArray(int size) {
            return new LiveToVodFullConfiguration[size];
        }
    };
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        if(this.linearAssets != null) {
            dest.writeInt(this.linearAssets.size());
            dest.writeList(this.linearAssets);
        } else {
            dest.writeInt(-1);
        }
    }

    public LiveToVodFullConfiguration(Parcel in) {
        super(in);
        ...
        int linearAssetsSize = in.readInt();
        if( linearAssetsSize > -1) {
            this.linearAssets = new ArrayList<>();
            in.readList(this.linearAssets, LiveToVodLinearAssetConfiguration.class.getClassLoader());
        }
    }
}
